...
// SECURITY FIX (19 Aug 2026 audit): hardcoded fallback token/chat ID removed.
// Now REQUIRED env vars -- set the NEW rotated bot token in Railway's
// Variables tab (the old one is exposed and should be revoked via BotFather).
const TELEGRAM_TOKEN   = process.env.TELEGRAM_TOKEN;
const TELEGRAM_CHAT_ID = process.env.TELEGRAM_CHAT_ID;

if (!TELEGRAM_TOKEN || !TELEGRAM_CHAT_ID) {
  throw new Error(
    '[TELEGRAM] Missing required environment variable(s): ' +
    ['TELEGRAM_TOKEN', 'TELEGRAM_CHAT_ID'].filter(name => !process.env[name]).join(', ') +
